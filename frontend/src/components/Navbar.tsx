...
export const Navbar: React.FC = () => {
  ...
  // Hitung total item unik di keranjang untuk badge navigasi
  const cartCount = cart.reduce((acc, item) => acc + item.quantity, 0);

  // Fungsi pembantu untuk memeriksa apakah path saat ini aktif
  ...
  return (
    <nav className="bg-[#fbfbfa] border-b border-neutral-200 sticky top-0 z-50 px-8 py-5">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        {/* Menu Navigasi Tengah (Home, Explore, Cart berkumpul di sini) */}
        <div className="flex items-center space-x-10 text-xs font-semibold uppercase tracking-widest text-neutral-600">
          <Link
            to="/"
            className={`pb-1 transition hover:text-black border-b-2 ${
              isActive("/")
          ...
          <Link
            to="/cart"
            className={`relative pb-1 transition flex items-center space-x-1.5 hover:text-black border-b-2 ${
              isActive("/cart")
                ? "text-black border-black font-bold"
                : "border-transparent"
            }`}
